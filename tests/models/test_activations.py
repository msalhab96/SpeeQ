...
class TestSigmax:
    ...
    @pytest.mark.parametrize(
        ("input", "target", "dim"),
        (
            (torch.tensor([5.0]), torch.tensor([1.0]), 0),
            (torch.tensor([3.0, 3.0]), torch.tensor([0.5, 0.5]), 0),
            (torch.tensor([[3.0], [3.0]]), torch.tensor([1.0, 1.0]), 1),
            (
    ...
    def test_values(self, input, target, dim):
        ...
        act = activations.Sigmax(dim=dim)
        result = act(input)
        assert torch.allclose(result, target, rtol=1e-2, atol=1e-2)
