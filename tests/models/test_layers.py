...
class TestPredModule:
    ...
    @parameters_mark
    def test_forward(self, in_features, n_classes, batch_size, seq_len):
        ...
        model = self.module(
            in_features=in_features, n_classes=n_classes, activation=lambda x: x
        )
        result = model(input)
        shape = result.shape
...
class TestFeedForwardModule:
    ...
    def test_forward(self, batcher, d_model, ff_size, batch_size, seq_len, feat_size):
        """Tests the shape and the gradients of the model"""
...
class TestMultiHeadAtt:
    ...
    decoder_mask = torch.BoolTensor([[True, True], [True, False]])
    ...
    def test_reshape(self, batcher, d_model, h, masking_value, batch_size, seq_len):
    ...
    @pytest.mark.parametrize(
        ...
        ((24, 6, -1e15, masking_input, encoder_mask, decoder_mask, masking_expected),),
    )
    def test_mask(self, d_model, h, masking_value, att, key_mask, query_mask, expected):
    ...
    def test_with_no_mask(
        self, batcher, d_model, h, masking_value, batch_size, max_enc_len, max_dec_len
    ):
    ...
    def test_with_masks(
        self, batcher, d_model, h, masking_value, encoder_mask, decoder_mask
    ):
...
class TestMaskedMultiHeadAtt:
    ...
    key_mask2 = torch.BoolTensor([[True, True, False, False]])
    ...
    @pytest.mark.parametrize(
        ...
        ((12, 2, key_mask1, expected1), (12, 2, key_mask2, expected2)),
    )
...
class TestTransformerEncLayer:
    ...
    def test_forward(self, batcher, d_model, ff_size, h, batch_size, seq_len):
        ...
        model = layers.TransformerEncLayer(d_model=d_model, ff_size=ff_size, h=h)
        shape = (batch_size, seq_len, d_model)
        input = batcher(*shape)
        result = model(input)
        assert result.shape == shape
...
class TestGlobalMulAttention:
    @pytest.mark.parametrize(
        ("enc_size", "dec_size", "scaling_factor", "batch_size", "seq_len", "pad_lens"),
        (
            (8, 16, 1, 2, 7, [0, 3]),
            (8, 16, 1, 2, 7, None),
            (16, 16, 0.1, 1, 9, [4]),
    ...
    def test_forward(
        self, batcher, enc_size, dec_size, scaling_factor, batch_size, seq_len, pad_lens
    ):
        """Tests the returned shape and the gradients of the model's forward"""
        ...
        model = layers.GlobalMulAttention(
            enc_feat_size=enc_size,
            dec_feat_size=dec_size,
...
class TestConformerFeedForward:
    ...
    def test_forward(
        self, batcher, d_model, expansion_factor, p_dropout, batch_size, seq_len
    ):
        """Tests the returned shape and the gradients of the model's forward"""
        shape = (batch_size, seq_len, d_model)
        input = batcher(*shape)
...
class TestConformerRelativeMHSA:
    @pytest.mark.parametrize(
        ...
        ((16, 4, 0.0, 3, 10, [4, 5, 10]), (16, 4, 0.0, 3, 10, None)),
    )
    def test_forward(
        self, batcher, d_model, h, p_dropout, batch_size, seq_len, pad_len
    ):
        """Tests the returned shape and the gradients of the model's forward"""
        shape = (batch_size, seq_len, d_model)
        input = batcher(*shape)
        model = layers.ConformerRelativeMHSA(d_model=d_model, h=h, p_dropout=p_dropout)
        mask = None
...
class TestJasperResidual:
    ...
    def test_forward(self, batcher, in_channels, out_channels, seq_len, batch_size):
        """Tests the returned shape and the gradients of the model's forward"""
        input = batcher(batch_size, in_channels, seq_len)
        expected_shape = (batch_size, out_channels, seq_len)
...
class TestTransformerDecLayer:
    ...
    def test_forward(
        ...
        enc_mask = None
        dec_mask = None
        model = layers.TransformerDecLayer(d_model=d_model, ff_size=ff_size, h=h)
...
class TestPositionalEmbedding:
    ...
    def test_forward(self, int_batcher, vocab_size, batch_size, embed_dim, seq_len):
        """Tests the returned shape and the gradients of the model's forward"""
        expected_shape = (batch_size, seq_len, embed_dim)
        model = layers.PositionalEmbedding(vocab_size=vocab_size, embed_dim=embed_dim)
...
class TestSqueezeformerConvModule:
    ...
    def test_forward(
        self, batcher, d_model, kernel_size, p_dropout, batch_size, seq_len
    ):
        shape = (batch_size, seq_len, d_model)
        input = batcher(*shape)
...
class TestSqueezeAndExcit1D:
    @pytest.mark.parametrize(
        ("in_feature", "reduction_factor", "batch_size", "seq_len", "pad_lens"),
        (
            (16, 2, 3, 7, [5, 2, 0]),
            (16, 1, 3, 7, [5, 2, 0]),
            (16, 1, 3, 7, [0, 0, 0]),
    ...
    def test_forward(
        self, batcher, in_feature, reduction_factor, batch_size, seq_len, pad_lens
    ):
